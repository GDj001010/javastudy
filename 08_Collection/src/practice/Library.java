...
public class Library {
	...
	private void deleteBook() {	// RuntimeException은 UnChecked-Exception이므로 throws RuntimeException을 생략할 수 있다.
		System.out.println("=== 책 삭제하기 ===");
			...
			if(isbn.isEmpty() == false) {
				...
				//	1. 객체 기반 삭제(Book 객체 간 동등 비교가 필요하기 때문에 equals 메소드를 오버라이드(@Override) 해야 한다.
				for(Book book : books) {
				...
				/* 2. 인덱스 기반 삭제	
				for(int i = 0; i < books.size(); i++) {		// 향상for문을 안 쓰고 일반 for문을 사용하는 이유는 인덱스 정보 때문이다.
					Book book = books.get(i);				// 삭제할 때 인덱스가 필요하기 때문
					if(isbn.equals(book.getIsbn())) {
	...
	private void findBook() throws RuntimeException{	// RuntimeException은 UnChecked-Exception이므로 throws RuntimeException을 생략할 수 있다.
	...
	private void printAllBooks() throws RuntimeException{	// RuntimeException은 UnChecked-Exception이므로 throws RuntimeException을 생략할 수 있다.
