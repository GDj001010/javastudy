...
public class MainClass {
	 ...
	 */
	 ...
	 public static void main(String[] args) {
		 ...
		 // 변수 선언하기(어떤 데이터를 저장할 것인가? 어떤 이름을 사용한 것인가?)
		 
		 // 타입의 구분 (변수)
		 // 1. primitive type : 기본 타입 (소문자로 되어 있음), 값을 저장하는 타입.
		 // 2. reference type : 참조 타입 (첫 글자가 대문자로 되어 있음, 모든 클래스는 참조 타입), 값이 저장된 참초값을 저장하는 타입
		 
		 
		 // 논리 타입 (참, 거짓 저장)  is로 시작하는게 많다 
		 boolean isGood = true;
		...
		// 문자열 타입 (여러 글자를 의미, 글자 수의 제한이 없음) 참초 타입
		 String name = "김동형";  // java에서 기본 타입과 같은 사용법을 제공
