...
public class MainClass {
	...
	/*
		Writer
		1. 문자 기반의 출력 스트림이다. ( 출력 = 데이터를 보냄 ) (입력 = 입력을 받음 )
		2. Writer로 끝나는 이름의 클래스는 모두 문자 기반의 출력 스트림이다.
		3. 문자를 내보내는 스트림이다. (파일로 문자를 보낸다. 서버로 문자를 보낸다.	등등 어디론가 문자를 보낼 때 사용)
	*/
	public static void ex01() {
		...
		try {
		// 출력 스트림 생성
		fw = new FileWriter(file);	// ex01_txt 파일에 문자를 보내줄 통로에 연결해주는 코드, 반드시 예외 처리가 필요한 코드
			...
			e.printStackTrace();
		}finally {				// 예외가 발생하든 안 하든 마지막에 실행되는 코드
			try {
				if(fw != null) {
	...
	public static void ex03() {
		...
		/*
			BufferedWriter - 자체 기억장소를 가진 라이터
	...
	public static void ex05(){
		...
		/*
			...
			2. 줄 바꿈이 자주 필요한 경우에 println() 메소드를 사용하기 위해서 사용한다.
		...
		*/
		...
		try {
			...
			out.close();		// close 메소드 내부에서 이미 IOException을 처리하고 있기 때문에 try-catch를 한 번 더 안 해줘도 됨
		}
