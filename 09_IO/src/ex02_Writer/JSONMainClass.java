...
public class JSONMainClass {
	...
	/*
		...
		3. 프로퍼티(property)와 값(value)으로 데이터를 구성한다.  - Map이랑 비슷하다
	*/
	/*
	...
	*/
		// {"model":"세탁기","maker":"삼성","price":100}  밖에다가 쓰고 복붙
		// {"name":"james","age":30,"height":180.5,"isAlive":true}
		
		// String 형식의 JSON 데이터 (객체)  {}
	...
	public static void ex05() {
		...
		try(BufferedWriter bw = new BufferedWriter(new FileWriter(file))){
			bw.write(content);
			System.out.println("product.json 파일이 생성됨.");
		}catch(IOException e) {
			e.printStackTrace();
		}
		...
		try(BufferedWriter bw = new BufferedWriter(new FileWriter(file))){
			for(Object ob : arr) {
				...
				bw.write(obj.getString("model"));
				bw.write(obj.getString("maker"));
				bw.write(obj.getInt("price"));
				System.out.println("product.json 파일이 생성됨.");
			}
		}
		*/
