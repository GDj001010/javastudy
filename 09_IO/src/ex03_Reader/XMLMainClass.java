...
public class XMLMainClass {
	...
	public static void main(String[] args) {
		...
		try {
			...
			Document document = builder.parse(file);			// product.xml 을 분석(파싱)한 Document 객체
			...
			for(int i = 0; i < nodeList.getLength(); i++) {
				Node node = nodeList.item(i);		// Node는 #text(줄바꿈을 의미)와 <product> 태그(Element 의미) 중 하나이다.
				if(node.getNodeType() == Node.ELEMENT_NODE)	{		// Node가 <product> 태그인가 점검하는 코드
					Map<String, Object> product = new HashMap<String, Object>();
					NodeList nodeList2 = node.getChildNodes();
					for(int j = 0; j < nodeList2.getLength(); j++) {
						Node node2 = nodeList2.item(j);				// node2는 #text(줄바꿈을 의미)와 <model>, <maker>, <price> 태그(Element를 의미) 중 하나이다.
						if(node2.getNodeType() == Node.ELEMENT_NODE) {	// node2가 <modle>, <maker>, <price> 중 하나인가
